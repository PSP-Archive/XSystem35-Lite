...
static void get_filemap(drifiles *d, FILE *fp) {
	...
	_b = (char*)malloc(sizeof(char)*(mapsize << 8));
	...
	/* map of disk */
	d->map_disk = (char*)malloc(sizeof(char)*d->maxfile);
	/* map of data in disk */ 
	d->map_ptr  = (short*)malloc(sizeof(short)*d->maxfile);
...
static void get_fileptr(drifiles *d, FILE *fp, int disk) {
	...
	/* allocate read buffer */
	_b = (char*)malloc(sizeof(char)*(ptrsize << 8));
	...
	d->fileptr[disk] = (int*)calloc(sizeof(int), filecnt);
...
drifiles *dri_init(char **file, int cnt, boolean mmapping) {
	drifiles *d = (drifiles*)calloc(sizeof(drifiles), 1);
	FILE *fp;
	int i;
...
dridata *dri_getdata(drifiles *d, int no) {
	...
	if (d->mmapped) {
		...
		FILE *fp;
		data = (char*)malloc(sizeof(char)*readsize);
	...
	dfile = (dridata*)calloc(sizeof(dridata), 1);
