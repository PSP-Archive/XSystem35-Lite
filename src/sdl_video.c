...
int sdl_Initilize(void) {
	sdl_videodev = (struct sdl_private_data*)calloc(sizeof(struct sdl_private_data), 1);
...
#ifdef DEPTH
	sdl_vflag = SDL_SWSURFACE | SDL_ANYFORMAT;
	
	sdl_display = SDL_SetVideoMode(SYS35_DEFAULT_WIDTH, SYS35_DEFAULT_HEIGHT, DEPTH, sdl_vflag);
...
static void makeDIB(int width, int height, int depth) {
	...
	sdl_dibinfo = (agsurface_t*)calloc(sizeof(agsurface_t), 1);
