...
*/
static qnt_header *extract_header(BYTE *b) {
	qnt_header *qnt = (qnt_header*)malloc(sizeof(qnt_header)* 1);
	int rsv0;
...
static void extract_pixel(qnt_header *qnt, BYTE *pic, BYTE *b) {
	...
	BYTE *raw = (BYTE*)malloc(sizeof(BYTE)* ucbuf);
...
cgdata *qnt_extract(BYTE *data) {
	...
	cg->pic = (BYTE*)malloc(sizeof(BYTE)* (qnt->width+10) * (qnt->height+10) * 3);
	extract_pixel(qnt, cg->pic, data + qnt->hdr_size);
	
	if (qnt->alpha_size != 0) {
		cg->alpha = (BYTE*)malloc(sizeof(BYTE)* (qnt->width+10) * (qnt->height+10));
