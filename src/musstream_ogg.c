...
WAVFILE *ogg_getinfo(char *data, long size) {
	...
	o2p = (ogg2pcm_t*)malloc(sizeof(ogg2pcm_t)* 1);
	o2p->top = o2p->cur = data;
	o2p->size = size;
	
	vf = (OggVorbis_File*)malloc(sizeof(OggVorbis_File)* 1);
	
	if (0 > ov_open_callbacks(o2p, vf, NULL, 0, ovcb)) {
		goto eexit;
	}
	wfile = (WAVFILE*)calloc(sizeof(WAVFILE), 1);
...
musstream_t *ms_ogg(WAVFILE *wfile) {
	musstream_t *ms = (musstream_t*)calloc(sizeof(musstream_t), 1);
