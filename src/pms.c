...
*/
static pms_header *extract_header(BYTE *b) {
	pms_header *pms = (pms_header*)malloc(sizeof(pms_header)* 1);
...
cgdata *pms256_extract(BYTE *data) {
	cgdata *cg = (cgdata*)calloc(sizeof(cgdata), 1);
	pms_header *pms = extract_header(data);
	
	cg->pal = (Pallet256*)malloc(sizeof(Pallet256)* 1);
	getpal(cg->pal, data + pms->pmsPp);
	
	/* +10: margin for broken cg */
	cg->pic = (BYTE*)malloc(sizeof(BYTE)* (pms->pmsXW + 10) * (pms->pmsYW + 10));
...
cgdata *pms64k_extract(BYTE *data) {
	...
	cg->alpha = NULL;
	if (pms->pmsPp != 0) {
		cg->alpha = (BYTE*)malloc(sizeof(BYTE)* (pms->pmsXW + 10) * (pms->pmsYW + 10));
